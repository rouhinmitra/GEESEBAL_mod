...
def albedo_correction(image):
    alfa=image.expression(
        'ALFA+0.32482318838459845-0.0010375226087962139*LST',{
...
def fexp_radshort_down(image, z_alt, T_air, UR,SUN_ELEVATION):
    ...
    tao_sw = image.expression(
       ...
       'cos_theta' : cos_theta}).rename('Tao_sw')
##Correction of albedo 
    tao_sw_corr=image.expression(
         'Tao_sw-0.9907739975138314+0.003295752273228452*LST',
         {"Tao_sw":image.select("Tao_sw"),
          "LST":image.select("ST_B10")}
     ).rename("tao_sw_corr")
    ...
    #ADD BANDS
    image =  image.addBands([Rs_down, tao_sw,es,ea,tao_sw_corr]);
    return image
...
def fexp_radlong_down(image, n_Ts_cold):
    ...
    return image
def fexp_radlong_down_corr(image, n_Ts_cold):
    Rl_down = image.expression(
       '(0.83-0.18*(10**(-0.67*ea)))*stefBol* (n_Ts_cold ** 4) ',{
       'ea' : image.select("EA"),
       'stefBol': ee.Number(5.67e-8),
       'n_Ts_cold' : n_Ts_cold}).rename('Rl_down_corr')
...
def fexp_radbalance(image):
    ...
    Rn = image.expression(
       ...
       'alfa' : image.select('ALFA_corr'),
       'Rs_down': image.select('Rs_down'),
       'Rl_down' : image.select('Rl_down_corr'),
