...
def fexp_spec_ind(image):
   ...
   ## lst = image.expression(
    ...
    #ADD BANDS
    image = image.addBands([ndvi,evi,savi,lst,lai,e_0,e_NB,coords,ndvi_neg,pos_ndvi,int_, sd_ndvi, ndwi,lst_neg,lst_nw]);
    return image
...
def LST_DEM_correction(image, z_alt, T_air, UR,SUN_ELEVATION,hour,minuts):
    ...
    B=(ee.Number(360).divide(ee.Number(365))).multiply(ee.Number(doy).subtract(ee.Number(81)));
    ...
    gamma = (slope_aspect.select('aspect').subtract(ee.Number(180))).multiply(degree2radian)
    ...
    HOUR_A = (Local_hour_time.subtract(ee.Number(12))).multiply(15);
    ...
    #ADD BANDS
    image=image.addBands([TS_DEM, lst_neg, lst_nw,tao_sw,ea,W])
...
def fexp_radshort_down(image, z_alt, T_air, UR,SUN_ELEVATION):
    ...
    # Calculate Shine transmissivity
    T_shine = image.expression(
        'cos_theta / (1.2 * cos_theta + (ea * (cos_theta + 1) * 0.01) + 0.0455)', {
        'cos_theta': cos_theta,
        'ea': ea
    }).rename('T_shine')

    # Calculate Rs_shine
    Rs_shine = T_shine.multiply(gsc).multiply(cos_theta).rename('Rs_down')

    # Add bands
    image = image.addBands([T_shine, Rs_shine])
    return image
...
def fexp_radlong_down(image, n_Ts_cold):
    ...
    emis = image.expression(
        '0.83 - 0.18 * (10 ** (-0.67 * ea))', {
        'ea': image.select('EA')
    }).rename('emis')

    sig = ee.Number(5.670374419e-8)

    Rl_down = image.expression(
        'emis * sig * (n_Ts_cold ** 4)', {
        'emis': emis,
        'sig': sig,
        'n_Ts_cold': n_Ts_cold
    }).rename('Rl_down')
...
# Replace the sensible heat flux function with the NDVI-adjusted dT version

def fexp_sensible_heat_flux(image, ux, UR, Rn24hobs, n_Ts_cold, d_hot_pixel, d_cold_pixel, date_string, refpoly, NDVI_soil, NDVI_veg, use_static_zom=False, vegetation_height=3):
    print(f"DEBUG_SHF: Starting sensible heat flux function")
    print(f"DEBUG_SHF: NDVI_soil = {NDVI_soil}, NDVI_veg = {NDVI_veg}")
    
    # Convert NDVI_soil and NDVI_veg to ee.Number objects
    NDVI_soil = ee.Number(NDVI_soil)
    NDVI_veg = ee.Number(NDVI_veg)
    
    n_veg_hight = ee.Number(vegetation_height)
    n_zx = ee.Number(2)
    ...
    #TS COLD PIXEL
    n_Ts_cold = ee.Number(n_Ts_cold)
    n_G_cold = ee.Number(d_cold_pixel.get('G').getInfo())
    n_Rn_cold = ee.Number(d_cold_pixel.get('Rn').getInfo())
    n_long_cold = ee.Number(d_cold_pixel.get('x').getInfo())
    n_lat_cold = ee.Number(d_cold_pixel.get('y').getInfo())
    #POINT GEOMETRY
    p_cold_pix =  ee.Geometry.Point([n_long_cold, n_lat_cold])
    # Safely get hot pixel values with validation
    try:
        ...
        if (n_Ts_hot.getInfo() is None or n_G_hot.getInfo() is None or 
            n_Rn_hot.getInfo() is None or n_long_hot.getInfo() is None or 
            n_lat_hot.getInfo() is None):
            print("Error: One or more hot pixel values are None")
            raise ValueError("Invalid hot pixel values")
            
    except Exception as e:
        print(f"Error extracting hot pixel values: {e}")
        raise ValueError(f"Failed to extract hot pixel values: {e}")
    ...
    i_u200 = i_savi.expression(
      'i_ufric_ws *  (log(n_hight/n_zom)/n_K)', {
              ...
              'n_K' : n_K}).rename('u200')
    if use_static_zom:
        i_zom = ee.Image.constant(0.05).rename('zom')
    else:
    ...
    print(f"DEBUG_SHF: Starting iterative calculation with NDVI-based dT correction")
    n_ro_hot= (ee.Number(-0.0046).multiply(n_Ts_hot)).add(ee.Number(2.5538))
    n_ro_cold = (ee.Number(-0.0046).multiply(n_Ts_cold)).add(ee.Number(2.5538))
    n_H_hot = ee.Number(n_Rn_hot).subtract(ee.Number(n_G_hot))
    Beta = ee.Number(0.05)
    n_H_veg = (ee.Number(n_Rn_cold).subtract(ee.Number(n_G_cold))).multiply(Beta)
    ...
    for n in range(10):
        ...
        # Cold Pixel
            # Cold Pixel
        d_rah_cold = i_rah.reduceRegion(
                reducer= ee.Reducer.first(),
                geometry= p_cold_pix,
                scale= 30,
                maxPixels=9000000000)
        n_rah_cold =   ee.Number(d_rah_cold.get('rah'))

        n_dT_hot = (n_H_hot.multiply(n_rah_hot)).divide(n_ro_hot.multiply(n_Cp))
        n_dT_cold = ee.Number(0)
        dT_veg = (n_H_veg.multiply(n_rah_cold)).divide(n_ro_cold.multiply(n_Cp))

        # Check for division by zero in coefficient calculation
        temp_diff = n_Ts_cold.subtract(n_Ts_hot)
        if temp_diff.getInfo() == 0:
            print("Error: Temperature difference between cold and hot pixels is zero")
            raise ValueError("Cannot calculate coefficients: temperature difference is zero")
            
        n_coef_a = (n_dT_cold.subtract(n_dT_hot)).divide(temp_diff)
        n_coef_b = n_dT_hot.subtract(n_coef_a.multiply(n_Ts_hot))
        
        # Print coefficient and rah values at each iteration
        print(f"DEBUG_SHF: Iteration {n+1}")
        # print(f"DEBUG_SHF:   rah_hot = {n_rah_hot.getInfo():.6f}")
        # print(f"DEBUG_SHF:   coef_a = {n_coef_a.getInfo():.6f}")
        # print(f"DEBUG_SHF:   coef_b = {n_coef_b.getInfo():.6f}")
        # print(f"DEBUG_SHF:   dT_hot = {n_dT_hot.getInfo():.6f}")
        
        i_lst_med = image.select('T_LST_DEM')
        i_ndvi = image.select('NDVI')
        epsilon = ee.Number(1e-6)
        # Fixed fv calculation with proper ee.Number objects
        fv = i_ndvi.subtract(NDVI_soil).divide(NDVI_veg.subtract(NDVI_soil)).clamp(0,1)
        fstress = i_lst_med.subtract(n_Ts_cold).divide(n_Ts_hot.subtract(n_Ts_cold)).clamp(0,1)
        # Convert coefficients to ee.Image for use in expressions
        i_coef_a = ee.Image.constant(n_coef_a)
        i_coef_b = ee.Image.constant(n_coef_b)
        i_dT_soil = ee.Image(0).clip(refpoly).expression(
            '(n_coef_a * i_lst_med) + n_coef_b', {
            'n_coef_a' : n_coef_a,
            'n_coef_b': n_coef_b,
            'i_lst_med':i_lst_med }).rename('dT_soil')
        i_dT_fv = ((ee.Image(1).subtract(fv)).multiply(i_coef_a.multiply(i_lst_med).add(i_coef_b))).add(fv.multiply(dT_veg)).rename('dT_fv')
        i_dT_int =  ((ee.Image(1)).subtract(fstress)).multiply(i_dT_fv).add(fstress.multiply(i_dT_soil)).rename('dT')
        ...
        i_psih_01 = img.expression(
                '-5*(hight/i_L_int)',{'hight' : ee.Number(0.1),'i_L_int': i_L_int})
    ...
    # Add fv as a band
    fv_band = fv.rename('fv')
    fstress_band = fstress.rename('fstress')
    # Cast coefficients to float to ensure consistent type across images
    coef_a_band = i_coef_a.float().rename('coef_a')
    coef_b_band = i_coef_b.float().rename('coef_b')
    image = image.addBands([i_H_final, i_rah_final, i_dT_final,
                            i_rah_first,image.select('zom'),image.select('u_fr'),i_ufric,coef_a_band,coef_b_band,fv_band,fstress_band])
    print(f"DEBUG_SHF: Completed iterative calculation with NDVI-based dT correction")
    return image 